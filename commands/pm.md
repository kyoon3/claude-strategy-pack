...
> Run a source-of-truth reconciliation. Treat `BUSINESS.md` / `STRATEGY.md` and `ROADMAP.md` / `PLAN.md` as authoritative (unless I name a different source below). Read them first, then scan the TODO/backlog family: `TODO.md`, any `todo-*.md` / `BACKLOG-*.md` under `.claude/rules/` `docs/` or repo root, and API-contract docs. Pull `git log --oneline -20` + `git log --since='2 weeks ago' --name-only` over docs paths for drift signal. Do NOT scan or edit plan/spec files — they're out of scope (read one only if a backlog task references it and you need the scope). Follow your Reconciliation contract: diagnose first (produce the report), then EDIT the TODO/backlog docs so they agree with the source — never edit the source docs or plan files, never touch code/tests/hooks. Pick the dimensions where the source↔downstream gap is widest. If nothing needs reconciling, return the report without forcing edits. Follow the output format in your system prompt exactly.
